...
public class ControladorPieza implements ActionListener, ItemListener {
	...
	public ControladorPieza(VentanaAltaModifPieza ventana, Modelo modelo, PiezaDTO pieza, tipo_vista tipo) throws Exception {
		iniciar(ventana, modelo);
		this.setPieza(pieza);
		this.ventana.getBtnAlta().setText("Guardar cambios");
		this.ventana.setTitle("Modificar pieza " + " " + pieza.getMarca() + " - " + pieza);
		cargarPieza();
		...
		case VER:
			bloquearCampos();
			this.ventana.setTitle("Detalle pieza" + " " + pieza.getMarca() + " - " + pieza);
			break;
		...
		case REAJUSTE_STOCK:
			...
			this.ventana.getSpinnerBajoStock().setVisible(true);
			this.ventana.getTfBajoStock().setVisible(false);
...
//		this.ventana.getSpinnerBajoStock().setModel(new SpinnerNumberModel(new Integer(pieza.getBajo_stock()), new Integer(pieza.getBajo_stock()), new Integer(pieza.getBajo_stock()), new Integer(1)));
		this.ventana.getSpinnerBajoStock().setVisible(false);
		this.ventana.getTfBajoStock().setVisible(true);
		this.ventana.getTfBajoStock().setText(String.valueOf(pieza.getBajo_stock()));
	}
	...
	private void cargarPieza() {
		...
		this.ventana.getSpinnerBajoStock().setModel(new SpinnerNumberModel(new Integer(pieza.getBajo_stock()), new Integer(1), null, new Integer(1)));
	}
		...
		if (e.getSource() == this.ventana.getBtnAlta()) {
			if (this.ventana.getBtnAlta().getText().equals("Reajustar")) {
				...
				if (stockNuevo > this.pieza.getStock()) {
					...
					try {
						modelo.bajaStockPieza(pieza, this.pieza.getStock() - stockNuevo, 4);
						// TODO estado 4 = perdida
					} catch (Exception e1) {
						e1.printStackTrace();
					}
				}
				JOptionPane.showMessageDialog(null, "El stock se ha reajustado correctamente", "Reajuste de stock", JOptionPane.INFORMATION_MESSAGE);
				this.ventana.dispose();
			} else {
				if (camposCorrectos()) {
					if (this.pieza == null) { // Es alta
						this.pieza = new PiezaDTO(0, (MarcaDTO) this.ventana.getCboMarcas().getSelectedItem(), this.ventana.getTfIdUnico().getText(), this.ventana.getTfDescripcionMarca().getText(), Float.parseFloat(this.ventana.getTfPrecioVenta().getText()), Integer.parseInt((ventana.getSpinnerBajoStock().getModel().getValue().toString())));

						altaPieza();
					} else { 
						// Es modificacion edito la pieza
						modificacionPieza();
					}
					this.ventana.dispose();
				} else {
	...
	private void modificacionPieza() {
		try {
			if (camposCorrectos()) {
				ActualizarPieza();
				JOptionPane.showMessageDialog(null, "La pieza " + ventana.getCboMarcas().getModel().getSelectedItem() + ", " + ventana.getTfDescripcionMarca().getText() + " " + "se ha modificado satisfactoriamente.", "Pieza actualizada", JOptionPane.INFORMATION_MESSAGE);
			}
		} catch (Exception e) {
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "Hubo un error al modificar la pieza", "Pieza no actualizada", JOptionPane.ERROR_MESSAGE);
		}
	}

	private void ActualizarPieza() throws Exception {
		try {
			PiezaDTO pieza = new PiezaDTO(this.pieza.getIdProdPieza(), ((MarcaDTO) ventana.getCboMarcas().getModel().getSelectedItem()), this.ventana.getTfIdUnico().getText(), this.ventana.getTfDescripcionMarca().getText(), Float.parseFloat(this.ventana.getTfPrecioVenta().getText()), Integer.parseInt((ventana.getSpinnerBajoStock().getModel().getValue().toString())));
			this.modelo.modificarPieza(pieza);
	...
	private void altaPieza() {
		try {
			...
			JOptionPane.showMessageDialog(null, "La pieza, " + ventana.getTfDescripcionMarca().getText() + " " + "se ha creado satisfactoriamente.", "Nueva pieza creada", JOptionPane.INFORMATION_MESSAGE);
		} catch (Exception e) {
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "Hubo un error al crear la pieza " + e.getMessage(), "La pieza no se ha creado", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private boolean camposCorrectos() {
		...
		return !this.ventana.getTfDescripcionMarca().getText().isEmpty() && !this.ventana.getTfPrecioVenta().getText().isEmpty() && !this.ventana.getTfIdUnico().getText().isEmpty() && this.ventana.getTfIdUnico().getText().length() == 8;
	}
