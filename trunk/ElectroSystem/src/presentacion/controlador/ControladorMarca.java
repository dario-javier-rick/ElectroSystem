		...
		if (campoCorrecto()) {
			if (!marcaExiste()) {
				if (this.marca == null) {
					...
					JOptionPane.showMessageDialog(null, "Se ha agregado una nueva marca.", "Nuevo marca creada", JOptionPane.INFORMATION_MESSAGE);
				} else {
					crearActualizarMarca(this.marca.getIdMarca());
					JOptionPane.showMessageDialog(null, "Se ha actualizado la marca stisfactoriamente.", "Marca actualizada", JOptionPane.INFORMATION_MESSAGE);
				}
				this.ventana.dispose();
			} else {
				JOptionPane.showMessageDialog(null, "Ya existe una marca con ese nombre.", "Error al crear/actualizar", JOptionPane.WARNING_MESSAGE);
			}
		} else {
