...
public class ControladorCliente implements ActionListener, ItemListener {
	...
	public ControladorCliente(VentanaAltaModifCliente ventana, Modelo modelo, ClienteDTO cliente, boolean editable, boolean editarDireccion, UsuarioDTO usuarioLogueado) {
		iniciar(ventana, modelo);
		this.setCliente(cliente);
		this.setUsuario(usuarioLogueado);
	...
	private void iniciar(VentanaAltaModifCliente ventana, Modelo modelo) {
		...
		try {
			...
			this.ventana.getComboLocalidades().addItemListener(this);

			this.ventana.getComboLocalidades().addItem(new LocalidadDTO(0, "(seleccionar)", new ZonaDTO(0, null, 0), null));
	...
	private void bloquearCampos() {
		this.ventana.setTitle("Cliente: (" + this.cliente.getIdCliente() + ") - " + this.cliente);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == this.ventana.getBtnAlta()) {
			...
			new ControladorAdminOT(ventanaOT, modelo, cliente, usuario);
			...
			if (controladorLocalidad.getElegida() != null && !controladorLocalidad.getElegida().getNombre().equals("(seleccionar)")) {
				...
				this.ventana.getTfProvincia().setText(controladorLocalidad.getElegida().getProvincia());
			}
		}
	}
	...
	private void modificacionCliente(int idCliente) {
		try {
			...
			JOptionPane.showMessageDialog(null, "El cliente " + ventana.getTfNombre().getText() + " " + ventana.getTfApellido().getText() + " " + "se ha modificado satisfactoriamente.", "Cliente actualizado", JOptionPane.INFORMATION_MESSAGE);
		} catch (Exception e) {
			JOptionPane.showMessageDialog(null, "Hubo un error al modificar el cliente", "Cliente no actualizado", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	private void altaCliente() {
		try {
			...
			JOptionPane.showMessageDialog(null, "El cliente " + ventana.getTfNombre().getText() + " " + ventana.getTfApellido().getText() + " " + "se ha creado satisfactoriamente.", "Nuevo cliente creado", JOptionPane.INFORMATION_MESSAGE);
		} catch (Exception e) {
			JOptionPane.showMessageDialog(null, "Hubo un error al crear el cliente " + e.getMessage(), "El cliente no se ha creado", JOptionPane.ERROR_MESSAGE);
		}
	}

	private void crearActualizarCliente(int id) throws Exception {
		try {
			ClienteDTO nuevoCliente = new ClienteDTO(id, this.ventana.getTfNombre().getText(), this.ventana.getTfApellido().getText(), this.ventana.getTfDireccion().getText().isEmpty() ? "" : this.ventana.getTfDireccion().getText(), (LocalidadDTO) this.ventana.getComboLocalidades().getSelectedItem(), this.ventana.getTfTelefono().getText(), this.ventana.getTfMail().getText());
			if (id == 0)
				this.modelo.agregarCliente(nuevoCliente);
			else
				this.modelo.actualizarCliente(nuevoCliente);
			this.cliente = nuevoCliente;
			// para que lo devuelva si se lo pide y no ir a la db.
		} catch (Exception e) {
			throw e;
		}
	}
	...
	private boolean camposLlenos() {
		if (!this.ventana.getTfDireccion().getText().isEmpty() || !this.ventana.getComboLocalidades().getSelectedItem().toString().equals("(seleccionar)"))
			return !this.ventana.getTfNombre().getText().isEmpty() && !this.ventana.getTfApellido().getText().isEmpty() && !this.ventana.getTfTelefono().getText().isEmpty() && !this.ventana.getTfDireccion().getText().isEmpty()// Agregado
					&& !this.ventana.getComboLocalidades().getSelectedItem().toString().equals("(seleccionar)");
		else
			return !this.ventana.getTfNombre().getText().isEmpty() && !this.ventana.getTfApellido().getText().isEmpty() && !this.ventana.getTfTelefono().getText().isEmpty() && this.ventana.getTfDireccion().getText().isEmpty() && this.ventana.getComboLocalidades().getSelectedItem().toString().equals("(seleccionar)");
	}
	...
	public void itemStateChanged(ItemEvent e) {
		if (e.getSource() == this.ventana.getComboLocalidades()) {
			this.ventana.setTfProvincia(((LocalidadDTO) this.ventana.getComboLocalidades().getSelectedItem()).getProvincia());
		}
	}
}