...
public class ControladorUsuario implements ActionListener {
	...
	public ControladorUsuario(VentanaAltaModifUsuario ventana, Modelo modelo, List<UsuarioDTO> usuarios, UsuarioDTO usuario) {
		this.ventana = ventana;
		this.ventana.setLocationRelativeTo(null);
	...
	private void iniciarEditar() {
		...
		if (usuario.getRol().getIdRol() == 1) {
			try {
				...
				this.ventana.getLblMail().setVisible(true);
				this.ventana.getTfMail().setVisible(true);
				this.ventana.getTfMail().setText(modelo.obtenerMail());
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == this.ventana.getBtnCrear()) {
			...
			ventana.getLblFormatoDeMail().setVisible(false);
			if (usuario == null) {
							...
							if (coinciden(ventana.getPfPass().getPassword(), ventana.getPfRepitaPass().getPassword())) {
								if (!usuarioExiste(ventana.getTfUsuario().getText())) {
									if (usuario == null) {
										...
										if ((usuario.getRol().getIdRol() == 1 && Validador.mailValido(ventana.getTfMail().getText())) || usuario.getRol().getIdRol() != 1) {
	...
	private String generarUsuario() {
		...
		if (this.ventana.getRbAdm().isSelected())
			...
			tipo = "-tec";
		if (!this.ventana.getTfNombre().getText().isEmpty() && !this.ventana.getTfApellido().getText().isEmpty() && this.ventana.getTfNombre().getText().length() > 1) {
	...
	private void crearModificarUsuario(int id) {
		UsuarioDTO nuevo = new UsuarioDTO(id, ventana.getRbAdm().isSelected() ? 2 : 3, // TODO Hardcodeado.
				ventana.getRbAdm().isSelected() ? "Administrativo" : "Tecnico", // TODO Hardcodeado.
				ventana.getTfNombre().getText(), ventana.getTfApellido().getText(), ventana.getTfUsuario().getText(), String.valueOf(ventana.getPfPass().getPassword()));
		if (id == 0)
			try {
			...
			try {
				if (usuario.getRol().getIdRol() == 1)
					nuevo.setRol(new RolDTO(1, "Superusuario")); // TODO Hardcodeado por ahora.
				this.modelo.actualizarUsuario(nuevo);
				this.modelo.actualizarMailSuperusuario(this.ventana.getTfMail().getText());
			} catch (Exception e) {
				e.printStackTrace();
			}
	}
