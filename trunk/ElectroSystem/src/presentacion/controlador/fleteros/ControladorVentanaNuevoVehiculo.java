...
public class ControladorVentanaNuevoVehiculo implements ActionListener {
	...
	public ControladorVentanaNuevoVehiculo(VentanaNuevoVehiculo ventana, Modelo modelo, VehiculoDTO vehiculo, boolean edit) {
		this.modelo = modelo;
		this.ventana = ventana;
		this.ventana.setLocationRelativeTo(null);
		traerDatos(vehiculo);
	...
	public void traerDatos(VehiculoDTO vehiculo) {
		...
		DateModel<Calendar> modelVTV = (DateModel<Calendar>) this.ventana.getVencimientoVTV().getModel();
		modelVTV.setValue(vehiculo.getFechaVencimientoVTV());
		if (vehiculo.isOblea()) {
			this.ventana.getRdbtnSi().setSelected(true);
			DateModel<Calendar> modelOblea = (DateModel<Calendar>) this.ventana.getVencimientoOblea().getModel();
			modelOblea.setValue(vehiculo.getFechaVencimientoOblea());
		} else
			this.ventana.getRdbtnNo().setSelected(true);
	...
	public void crearVehiculo() {
		try {
			VehiculoDTO nuevo = new VehiculoDTO(this.ventana.getTxtPatente(), this.ventana.getTxtMarca(), this.ventana.getTxtModelo(), this.ventana.getSpinnerCapacidad(), (Calendar) this.ventana.getVencimientoVTV().getModel().getValue(), this.ventana.conOblea(), (Calendar) this.ventana.getVencimientoOblea().getModel().getValue(), this.ventana.getComboEstado());
	...
	public void modificarVehiculo() {
		VehiculoDTO nuevo = null;
		if (this.ventana.conOblea()) {
			nuevo = new VehiculoDTO(this.ventana.getTxtPatente(), this.ventana.getTxtMarca(), this.ventana.getTxtModelo(), this.ventana.getSpinnerCapacidad(), (Calendar) this.ventana.getVencimientoVTV().getModel().getValue(), true, (Calendar) this.ventana.getVencimientoOblea().getModel().getValue(), this.ventana.getComboEstado());
		} else {
			nuevo = new VehiculoDTO(this.ventana.getTxtPatente(), this.ventana.getTxtMarca(), this.ventana.getTxtModelo(), this.ventana.getSpinnerCapacidad(), (Calendar) this.ventana.getVencimientoVTV().getModel().getValue(), false, null, this.ventana.getComboEstado());
		}
		try {
	...
	public boolean camposCompletos() {
		if (this.ventana.getTxtPatente() == null || this.ventana.getTxtModelo() == null || this.ventana.getTxtMarca() == null || this.ventana.getVencimientoVTV().getModel().getValue() == null) {
			return false;
		}
		return true;
	}
