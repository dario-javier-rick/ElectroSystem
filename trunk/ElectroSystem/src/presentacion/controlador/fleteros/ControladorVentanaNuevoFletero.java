...
public class ControladorVentanaNuevoFletero implements ActionListener {
	...
	public ControladorVentanaNuevoFletero(VentanaNuevoFletero ventanaFletero, Modelo modelo, FleteroDTO fletero, boolean editable, UsuarioDTO usuario) {
		...
		this.fletero = new FleteroDTO(fletero.getIdFletero(), fletero.getNombre(), fletero.getApellido(), fletero.getCelular(), fletero.getRegistroConducir(), fletero.getVencimientoRegistro(), fletero.getVehiculo());
	...
	public void prepararVentanaEditar() {
		...
		DateModel<Calendar> modelReg = (DateModel<Calendar>) this.ventana.getVencimientoRegistro().getModel();
		modelReg.setValue(fletero.getVencimientoRegistro());
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == this.ventana.getBtnBuscarVehculo()) {
			...
			try {
				new ControladorBuscadorVehiculo(buscador, modelo, this.modelo.obtenerVehiculos(), this.logueado);
			} catch (Exception e2) {
				// TODO: handle exception
			}

			if (this.buscador.getVehiculoElegido() != null) {
				this.ventana.setTxtPatente(this.buscador.getVehiculoElegido().getPatente() + " - " + this.buscador.getVehiculoElegido().getMarca() + " " + this.buscador.getVehiculoElegido().getModelo());
			...
			// TODO que se muestre el ultimo vehiculo que se agrego para no
			// tener que buscarlo
			...
			if (fechasValidas()) {
				if (camposCompletos()) {
					if (vehiculoAsociado != null) {
						...
						JOptionPane.showMessageDialog(null, "No ha asociado ningún vehículo.");
					}
				}
			} else {
	...
	public boolean camposCompletos() {
		...
		if (this.ventana.getTxtNombre().isEmpty() || this.ventana.getTxtApellido().isEmpty() || this.ventana.getTxtCelular().isEmpty() || this.ventana.getTxtRegistro().isEmpty() || this.ventana.getVencimientoRegistro().getModel().getValue() == null || this.ventana.getTxtPatente() == null) {
	...
	public void crearFletero(int id) {
		...
		if (id != 0) {
			...
			FleteroDTO fletero = new FleteroDTO(this.fletero.getIdFletero(), this.ventana.getTxtNombre(), this.ventana.getTxtApellido(), this.ventana.getTxtCelular(), this.ventana.getTxtRegistro(), (Calendar) this.ventana.getVencimientoRegistro().getModel().getValue(),

					new VehiculoDTO(vehiculoAsociado.getPatente().toString(), vehiculoAsociado.getMarca().toString(), vehiculoAsociado.getModelo().toString(), vehiculoAsociado.getCapacidadCarga(), vehiculoAsociado.getFechaVencimientoVTV(), vehiculoAsociado.isOblea(), vehiculoAsociado.getFechaVencimientoOblea(), vehiculoAsociado.getEstado()));
			...
			FleteroDTO fletero = new FleteroDTO(0, this.ventana.getTxtNombre(), this.ventana.getTxtApellido(), this.ventana.getTxtCelular(), this.ventana.getTxtRegistro(), (Calendar) this.ventana.getVencimientoRegistro().getModel().getValue(),
	...
	public boolean fechasValidas() {
		...
		if (this.ventana.getVencimientoRegistro().getModel().getValue() == null)
			return false;
