...
public class ControladorElectrodomestico implements ActionListener {
	...
	private void iniciarEdicion() {
		this.ventana.setTitle("Modificar electrodoméstico " + electrodomestico.getMarca() + " - " + electrodomestico.getDescripcion());
			...
			if (camposCorrectos()) {
				if (!electrodomesticoExiste()) {
					if (electrodomestico == null) {
						...
						JOptionPane.showMessageDialog(null, "Se ha agregado un nuevo electrodoméstico.", "Nuevo electrodoméstico creado", JOptionPane.INFORMATION_MESSAGE);
					} else {
						crearActualizarElectrodomestico(electrodomestico.getIdElectro());
						JOptionPane.showMessageDialog(null, "Se ha actualizado el electrodoméstico.", "electrodoméstico actualizado", JOptionPane.INFORMATION_MESSAGE);
					}
					this.ventana.dispose();
				} else {
					JOptionPane.showMessageDialog(null, "Ya hay un electrodoméstico de esa marca con ese código.", "Error al crear/actualizar", JOptionPane.WARNING_MESSAGE);
				}
			} else {
	...
	private void crearActualizarElectrodomestico(int id) {
		ElectrodomesticoDTO nuevo = new ElectrodomesticoDTO(id, (MarcaDTO) this.ventana.getComboMarca().getSelectedItem(), this.ventana.getTfModelo().getText(), this.ventana.getTfNombre().getText());
	...
	private boolean camposLlenos() {
		return !this.ventana.getTfNombre().getText().isEmpty() && !this.ventana.getTfModelo().getText().isEmpty();
	}
