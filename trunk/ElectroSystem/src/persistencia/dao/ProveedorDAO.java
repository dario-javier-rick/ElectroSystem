...
public class ProveedorDAO {
	...
	private final String sp_InsertarIncumplidor = "call insertarProveedorIncumplidor(?,?,?)";
	...
	private final String sp_obtenerProveedorMasBarato = "call obtenerProveedorMasBarato(?,?,?)";
	...
	public void insertIncumplidor(ProveedorDTO proveedor) throws Exception {
		try {
			java.sql.CallableStatement cs = conexion.getSQLConexion().prepareCall(sp_InsertarIncumplidor);
	...
	public void delete(ProveedorDTO proveedor_a_eliminar, UsuarioDTO usuarioLogueado) throws Exception {
		try {
			...
			int idProveedor = proveedor_a_eliminar.getIdProveedor();
			cs.setInt(1, idProveedor);
			...
			List<MarcaDTO> marcas_a_borrar = proveedor_a_eliminar.getMarcas();
			for (MarcaDTO m : marcas_a_borrar) {
				borrarMarcaProveedor(idProveedor, m.getIdMarca());
			}
		} catch (SQLException e) {
			e.printStackTrace();
			throw e;
	...
	public ProveedorDTO obtenerProveedorMasBarato(PrecioPiezaDTO pieza) throws Exception {
		try {
			java.sql.CallableStatement cs = conexion.getSQLConexion().prepareCall(sp_obtenerProveedorMasBarato, ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY);

			cs.setInt(1, pieza.getPieza().getIdProdPieza());
			cs.registerOutParameter(2, java.sql.Types.INTEGER);
			cs.registerOutParameter(3, java.sql.Types.VARCHAR);
			...
			if (retCode == 0) {
				...
				return new ProveedorDTO(resultSet.getInt("IdProveedor"), resultSet.getString("Nombre"), resultSet.getString("Contacto"), resultSet.getString("Cuit"), resultSet.getString("Telefono"), resultSet.getString("Mail"), traerMarcasProvistas(resultSet.getInt("IdProveedor")));
			} else
				throw (new Exception(descErr));
