...
public class UsuarioDAO {
		...
		try {
			...
			if (retCode == 0) {
				while (resultSet.next()) {
					usuarios.add(new UsuarioDTO(resultSet.getInt("IdPersonal"), resultSet.getInt("IdRol"), resultSet.getString("Descripcion"), resultSet.getString("Nombre"), resultSet.getString("Apellido"), resultSet.getString("Usuario"), resultSet.getString("Pass")));
				}
			} else
	...
	public UsuarioDTO obtenerUsuario(String usuario) throws Exception {
		...
		try {
			...
			if (retCode == 0) {
				while (resultSet.next()) {
					objUsuario = new UsuarioDTO(resultSet.getInt("IdPersonal"), resultSet.getInt("IdRol"), resultSet.getString("Descripcion"), resultSet.getString("Nombre"), resultSet.getString("Apellido"), resultSet.getString("Usuario"), resultSet.getString("Pass"));
				}
			} else
				throw (new Exception(descErr));
	...
	public void actulizarMail(String mail) throws Exception {
		PreparedStatement statement;
		try {
			statement = conexion.getSQLConexion().prepareStatement("UPDATE acc_parametros SET parm_valor1 = ? WHERE parametro = 'MAIL_ADMIN'");
